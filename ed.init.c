/* $Header: /home/hyperion/mu/christos/src/sys/tcsh-5.20/RCS/ed.init.c,v 1.26 1991/03/13 16:25:39 christos Exp $ */
/*
 * ed.init.c: Editor initializations
 */
#include "config.h"
#ifndef lint
static char *rcsid = "$Id: ed.init.c,v 1.26 1991/03/13 16:25:39 christos Exp $";
#endif
...
# endif /* EXTPROC */
/*
 * Sun disables things differently 
 * (stty prints M-? if disable is -1)
 */
#ifdef sun
...
# endif /* BSDSIGS */
    ...
    /*
     * From: bret@shark.agps.lanl.gov (Bret Thaeler)
     * Avoid sunview bug, where a partially hidden window gets a SIG_WINDOW
     * every time the text is scrolled
     */
...
# endif /* TERMIO */
	...
	nio.c_lflag &= ~(NOFLSH | ECHOK | ECHONL | EXTPROC);
...
# endif
    ...
    /* if we have been called before but GotTermCaps isn't set,
       our TERM has changed, so get new termcaps and try again */

    if (!GotTermCaps)
	GetTermCaps();	/* does the obvious, but gets term type each time */
    ...
    xio.c_lflag &= ~(NOFLSH | ICANON | ECHO | ECHOE | ECHOK | ECHONL | EXTPROC |
		     IEXTEN);
    xio.c_lflag |= (ISIG);
...
int
Rawmode()			/* used to check and re-init the line. */
{				/* set the terminal into 1charatatime mode. */
    if (Tty_raw_mode)
	return(0);
...
# endif /* TERMIO */
    ...
    if (testio.c_cflag != nio.c_cflag) { /* prob. changed speeds */
...
# endif
	...
	(void) dosetkey((char *) &testio.c_cc[VKILL], (char *) &xio.c_cc[VKILL]);
    if (testio.c_cc[VEOF] != 1)	{ /* 'cause VEOL == VNUM, and 1 is raw mode */
...
# ifdef VEOL2
	...
	(void) dosetkey((char *) &testio.c_cc[VEOL2], (char *) &xio.c_cc[VEOL2]);
# endif /* VEOL2 */
# ifdef VSTART
...
# ifdef VSTOP
	...
	(void) dosetkey((char *) &testio.c_cc[VSTOP], (char *) &xio.c_cc[VSTOP]);
# endif /* VSTOP */
# ifdef VWERASE
    (void) dosetkey((char *) &testio.c_cc[VWERASE], (char *) &nio.c_cc[VWERASE]);
# endif /* VWERASE */
# ifdef VREPRINT
...
int
Cookedmode()			/* set tty in normal setup */
{
    sigret_t (*orig_intr)();
...
#if defined(TERMIO) || defined(POSIX)
    rawb = xio;
    rawb.c_lflag &= ~(IXON | ISIG | IEXTEN | PARENB);
# ifndef POSIX
