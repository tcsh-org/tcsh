/* $Header: /home/hyperion/mu/christos/src/sys/tcsh-5.20/RCS/ed.defns.c,v 1.8 1990/12/08 10:15:35 christos Exp $ */
/*
 * ed.defns.c: Editor function definitions and initialization
 */
#include "config.h"
#ifndef lint
static char *rcsid = "$Id: ed.defns.c,v 1.8 1990/12/08 10:15:35 christos Exp $";
#endif

#include "sh.h"
#include "ed.h"

static void ed_InitMetaBindings();
/* ed.defns -- CcFuncTbl and CcKeyMap defined */
...
#define		F_LIST_GLOB	70
    e_expand_history,
#define		F_EXPAND_HISNODE_ORY	71
    e_magic_space,
...
/* keymap table, each index into above tbl; should be 256*sizeof(KEYCMD)
   bytes long */
    ...
    F_UNASSIGNED,		/* M-^X */
    F_UNASSIGNED,		/* M-^Y */
    F_RUN_FG_EDINODE_OR,		/* M-^Z */
    F_COMPLETE,			/* M-^[ */
    F_UNASSIGNED,		/* M-^\ */
    F_UNASSIGNED,		/* M-^] */
    F_UNASSIGNED,		/* M-^^ */
    F_COPYPREV,			/* M-^_ */
    F_EXPAND_HISNODE_ORY,		/* M-SPACE */
    F_EXPAND_HISNODE_ORY,		/* M-! */
    F_UNASSIGNED,		/* M-" */
    F_UNASSIGNED,		/* M-# */
    F_CORRECT_L,		/* M-$ */
    F_UNASSIGNED,		/* M-% */
...
struct KeyFuncs FuncNames[] = {
    ...
    "expand-history",		F_EXPAND_HISNODE_ORY,
    ...
    "run-fg-editor",		F_RUN_FG_EDINODE_OR,
...
#ifdef DEBUG_EDIT
void
CheckMaps()			/* check the size of the key maps */
{
    int c1 = (256 * sizeof(KEYCMD));
    if ((sizeof(CcKeyMap)) != c1)
	CSHprintf ("CcKeyMap should be 256 entries, but is %d.\r\n",
		 (sizeof(CcKeyMap) / sizeof(KEYCMD)));

    if ((sizeof(CcAltMap)) != c1)
	CSHprintf ("CcAltMap should be 256 entries, but is %d.\r\n",
		 (sizeof(CcAltMap) / sizeof(KEYCMD)));

    if ((sizeof(CcEmacsMap)) != c1)
	CSHprintf ("CcEmacsMap should be 256 entries, but is %d.\r\n",
		 (sizeof(CcEmacsMap) / sizeof(KEYCMD)));

    if ((sizeof(CcViMap)) != c1)
	CSHprintf ("CcViMap should be 256 entries, but is %d.\r\n",
		 (sizeof(CcViMap) / sizeof(KEYCMD)));

    if ((sizeof(CcViCmdMap)) != c1)
	CSHprintf ("CcViCmdMap should be 256 entries, but is %d.\r\n",
...
ed_InitEmacsMaps()
{
    ...
    buf[0] = 030;
    buf[2] = 0;
    buf[1] = 030;  AddXkeyCmd(buf,F_EXCHANGE_MARK);
    buf[1] = '*';  AddXkeyCmd(buf,F_EXPAND_GLOB);
    buf[1] = '$';  AddXkeyCmd(buf,F_EXPAND_VARS);
